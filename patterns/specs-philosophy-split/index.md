...
Two doc trees: `specs/` (each rule once, no lengthy justification — read when *applying*) and `philosophy/` (tradeoffs, measured reality, named residual risks — read when *changing* the spec). Both READMEs state the contract from their side (philosophy: "only applying? skip this folder"); spec sections point at their justifying philosophy page; spec README carries "frozen decisions — don't re-litigate without going through philosophy". Fixes both failure modes: agents bloating on rationale, and agents changing rules they don't understand.
